...
DRAG_CONFIG_PATH=None
for arg in sys.argv[1:]:
 if arg not in PROFILES and os.path.isfile(arg) and arg.lower().endswith('.json'):
  DRAG_CONFIG_PATH=arg.replace('\\','/')
  break

if DRAG_CONFIG_PATH:
 RS["config_path"]=DRAG_CONFIG_PATH
...
def bp(m,i=""):
 ...
 ft={"folder":lambda:filedialog.askdirectory(initialdir=i or B),"file_py":lambda:filedialog.askopenfilename(initialdir=i or B,filetypes=[("Python/EXE","*.py *.exe"),("All","*.*")]),"file_json":lambda:filedialog.askopenfilename(initialdir=i or B,filetypes=[("JSON","*.json"),("All","*.*")]),"file_exe":lambda:filedialog.askopenfilename(initialdir=i or B,filetypes=[("EXE","*.exe"),("All","*.*")]),"save_json":lambda:filedialog.asksaveasfilename(initialdir=i or B,defaultextension=".json",filetypes=[("JSON","*.json"),("All","*.*")])}
 x=ft.get(m,lambda:filedialog.askopenfilename(initialdir=i or B))();r.destroy();return x or""
def pf():return json.dumps({"title":P["title"],"config_path":RS["config_path"],"columns":P["columns"],"hidden_cols":HC,"wide_cols":WC,"sort_col":P.get("sort_col"),"toggle_col":P.get("toggle_col"),"settings":RS,"default_config":DC,"ops_categories":list(DC.keys())},ensure_ascii=False)
...
function initModeSelect(){
 ...
 sel.innerHTML='';
 OPS_CATEGORIES.forEach(cat=>{
  const opt=document.createElement('option');
  opt.value=cat;opt.textContent=cat;
  sel.appendChild(opt);
 });
 if(OPS_CATEGORIES.length===0){
  const opt=document.createElement('option');opt.value='';opt.textContent='无模式';sel.appendChild(opt);
 }
}
function autoDetectMode(){
 const fn=currentConfigPath.replace(/\\/g,'/').split('/').pop().toLowerCase();
 ...
 if(OPS_CATEGORIES.length>0)return OPS_CATEGORIES[0];
 return null;
}
function onModeChange(){
 const sel=document.getElementById('modeSelect');
 currentOpsCategory=sel.value||null;
 renderRows();
}
function setModeFromPath(){
 const detected=autoDetectMode();
 if(detected){
  currentOpsCategory=detected;
  document.getElementById('modeSelect').value=detected;
 }
}
...
class Q(SimpleHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  if p=='/api/heartbeat':self._j({'ok':1})
  ...
  elif p=='/api/browse_save':
   i=b.get('initial','')
   if i and not os.path.isabs(i):i=os.path.join(B,i)
   if i and not os.path.exists(os.path.dirname(i) if i else ''):i=B
   self._j({'path':bp('save_json',os.path.dirname(i) if i else B)})
  elif p=='/api/new':
   ...
   cp=RS["config_path"]
   if mode:
    t_name=mode+"_temp.json"
    t_dir=os.path.dirname(cp) if os.path.dirname(cp) else '.'
    t=os.path.join(t_dir,t_name)
   else:
...
if __name__=='__main__':
 ec();port=5001
 while True:
  try:s=HTTPServer(('127.0.0.1',port),Q);break
  except OSError:port+=1
 print(f"[{P['title']}] http://127.0.0.1:{port}  (profile={A})");os.startfile(f"http://127.0.0.1:{port}");s.serve_forever()