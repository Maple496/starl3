...
async function loadConfig(){const j=await api('/api/load');if(!j.error){DATA={cols:PROFILE.columns.map(c=>c.name),rows:j.data.rows||[]};initModeSelect();setModeFromPath();renderTable();markClean();historyStack=[];updateUndoIndicator();updateUnsavedTag();}}
async function reloadConfig(){if(isNewUnsaved){showToast('新文件尚未保存，无法重载','info');return;}pushHistory();const j=await api('/api/reload',{path:currentConfigPath});if(!j.error){DATA={cols:PROFILE.columns.map(c=>c.name),rows:j.data.rows||[]};setModeFromPath();renderTable();markClean();showToast('已重载','success');}}
async function saveConfig(){collectAll();if(isNewUnsaved){doSaveAs();return;}const j=await api('/api/save',{data:{rows:DATA.rows},path:currentConfigPath});if(!j.error){markClean();showToast('已保存','success');}}
async function doSaveAs(){const j=await api('/api/browse_save',{initial:currentConfigPath});if(j.path){const r=await api('/api/save',{data:{rows:DATA.rows},path:j.path});if(!r.error){currentConfigPath=j.path;runtimeSettings.config_path=j.path;document.getElementById('configPathLabel').textContent=j.path;isNewUnsaved=false;updateUnsavedTag();setModeFromPath();markClean();showToast('已保存到: '+j.path,'success');}}}
async function runExe(){collectAll();const j=await api('/api/run',{data:{rows:DATA.rows},settings:runtimeSettings,mode:currentOpsCategory||''});j.error?showToast(j.error):showToast(j.msg,'success');}
...
function openJsonEditor(){collectAll();const root=document.getElementById('modalRoot');root.innerHTML='<div class="modal-overlay"><div class="modal-box"><div class="modal-header"><h3>📝 JSON</h3></div><div class="modal-body"><textarea id="ja" spellcheck="false">'+esc(JSON.stringify(DATA.rows,null,2))+'</textarea></div><div class="modal-footer"><button class="modal-btn" onclick="applyJson()">应用</button><button class="modal-btn" onclick="closeModal()">取消</button></div></div></div>';}
function applyJson(){try{const o=JSON.parse(document.getElementById('ja').value);pushHistory();DATA.rows=Array.isArray(o)?o:(o.rows||[]);renderTable();markDirty();closeModal();}catch(e){alert(e);}}
...
function handleFileOpen(e){const f=e.target.files[0];if(!f)return;const rd=new FileReader();rd.onload=ev=>{try{const o=JSON.parse(ev.target.result);pushHistory();DATA.rows=Array.isArray(o)?o:(o.rows||[]);currentConfigPath=f.name;isNewUnsaved=false;updateUnsavedTag();document.getElementById('configPathLabel').textContent=f.name;setModeFromPath();renderTable();markClean();}catch(err){alert(err);}};rd.readAsText(f);e.target.value='';}

function newFile(){pushHistory();DATA.rows=[];currentConfigPath='新建文件.json';isNewUnsaved=true;updateUnsavedTag();document.getElementById('configPathLabel').textContent=currentConfigPath;setModeFromPath();renderTable();markDirty();historyStack=[];updateUndoIndicator();showToast('已新建，保存时将弹出另存为窗口','info');}
