# main_edit.py
import json, os, threading, webbrowser, sys, subprocess, importlib
...
importlib.reload(templates)  # 强制重新加载模板
        ...
        if path == '/api/heartbeat':
            ...
            for k in RUN_SETTINGS:
                if k in body.get('settings', {}):
                    val = body['settings'][k]
                    if k in ['config_path', 'exe', 'py', 'python_exe'] and isinstance(val, str): val = val.replace('\\', '/')
                    RUN_SETTINGS[k] = val
            resp = {"ok": 1}
            ...
            if not cmd_str: resp = {"error": "找不到执行程序"}
            else:
                with open(fp, 'w', encoding='utf-8') as f:
            ...
            if not os.path.isabs(src_path): src_path = os.path.join(BASE_DIR, src_path)
